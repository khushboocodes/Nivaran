 ...
 *    Used as the default and as the fallback when the OpenAI key is absent.
 *  - OpenAIService — calls the OpenAI Chat Completions REST API. Activated
 *    when `OPENAI_API_KEY` is set. Falls back to the heuristic on any
 *    network or parse failure so the citizen submission flow never breaks.
 *
 * Selection is controlled via env vars:
 *   AI_PROVIDER=openai|heuristic   (default: openai when key present, else heuristic)
 *   OPENAI_API_KEY=sk-...          (required for the openai provider)
 *   OPENAI_MODEL=gpt-4o-mini       (optional, defaults to a cheap fast model)
 *
...
/**
 * Decide which provider to use based on env vars. The OpenAI provider is
 * wrapped in a fallback that catches any error and returns the heuristic
...
function buildService(): AIService {
  ...
  const useOpenAI = !!apiKey && (explicit === 'openai' || explicit === '');
  ...
  const openai = makeOpenAIService(apiKey, model);
...
export async function* chatStream(messages: ChatMessage[]): AsyncIterable<string> {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.warn('[ai] chat fetch failed; falling back to heuristic.', err);
    yield heuristicChatReply(messages);
    return;
  }
  if (!res.ok || !res.body) {
    // eslint-disable-next-line no-console
    console.warn('[ai] chat upstream returned', res.status);
    yield heuristicChatReply(messages);
    return;
  }
  ...
  while (true) {
    ...
    while ((idx = buffer.indexOf('\n')) >= 0) {
      ...
      try {
        ...
        // Ignore malformed lines; OpenAI sometimes sends keepalives.
      }
    }
  }
}
